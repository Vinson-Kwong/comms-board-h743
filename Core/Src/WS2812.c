...
uint8_t buf[2048]; //4+700+7*24*(4*2)
uint16_t buf_len;
void Send_8bits(uint8_t dat)
{
    uint8_t i = 0;
    static uint8_t CodeOne[4] = {
        0xff,
        0xff,
        0x00,
        0x00,
    };
    static uint8_t CodeZero[3] = {0xfe, 0x00, 0x00};

    for (i = 0; i < 8; i++) {
        if ((dat & 0x80) == 0x80) {
            memcpy(buf + buf_len, CodeOne, sizeof(CodeOne));
            buf_len += sizeof(CodeOne);
        } else {
            memcpy(buf + buf_len, CodeZero, sizeof(CodeZero));
            buf_len += sizeof(CodeZero);
        }
        dat = dat << 1;
    }
}
...
void WS2812_update(uint8_t forceUpdate)
{
    if (WS2812_frameModified | forceUpdate) {
        ...
        buf_len = 4;
        for (uint8_t led = 0; led < WS2812_LED_FRAME_SIZE; led++) {
            Send_8bits(_WS2812talLedframe[led].FieldsIn.GREEN);
            Send_8bits(_WS2812talLedframe[led].FieldsIn.RED);
            Send_8bits(_WS2812talLedframe[led].FieldsIn.BLUE);
        }
        buf_len += 700;
        HAL_SPI_Transmit(WS2812_spiHandler, buf, buf_len, 10);
    }
...
// 效果1：全部LED亮度从小到大渐变
void WS2812_global_brightness_effect()
{
    const int delay = 10;
    uint8_t brightness = 0;
    uint32_t color_rgb[7] = {0xff0000, 0x00ff00, 0x0000ff, 0xffffff, 0xff0000, 0x00ff00, 0x0000ff};
    for (int led = 0; led < WS2812_LED_FRAME_SIZE; led++) {
        WS2812_setARGB(led, brightness, color_rgb[led]);
    }
    WS2812_update(1);
    HAL_Delay(delay);
}

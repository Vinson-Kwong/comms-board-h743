...
#endif /* __GNUC__ */
{
  ...
  HAL_UART_Receive(&hcom_uart[0], (uint8_t*)&ch, 1, HAL_MAX_DELAY);
  return ch;
}
#endif
/* USER CODE END PFP */
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
__attribute__ ((used,section(".freertos_heap_ucHeap"))) uint8_t ucHeap[ configTOTAL_HEAP_SIZE ];
/* USER CODE END 0 */
